...
- [ ] 3.1 Задеплоить фиксированным скриптом (`./deploy/deploy.sh`, при необходимости `--dirty`)
- [ ] 3.2 Проверить: оба `current` на VPS разрешаются в существующие директории нового релиза
- [ ] 3.3 Проверить: `curl -I https://miniapp.xdshka.party/` → 200, Mini App открывается в Telegram
- [ ] 3.4 Проверить: после prune новый релиз на месте, `KEEP_RELEASES` соблюдён
